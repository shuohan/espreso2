...
#         /data/smore_simu_same_fov/espreso2_sample_valid/sub-OAS30050_ses-d0110_T2w_initnorm_scale-4p9_fwhm-5.nii.gz
...
# images=($(ls -r /data/smore_simu_same_fov/espreso2_sample_valid/sub-OAS30050_ses-d0110_T2w_initnorm_scale-4p9_fwhm-2p45.nii.gz))

for image in ${images[@]}; do
    fwhm=$(echo $image | sed "s/.*\(fwhm-[0-9p]*\).*/\1/")
    scale=$(echo $image | sed "s/.*\(scale-[0-9p]*\).*/\1/")
    outdir=results_train_no-warmup/$(basename $image | sed "s/\.nii\.gz//")
    sp=$(echo $image | sed "s/\.nii\.gz/.npy/")
    # ../scripts/train.py -i $image -o $outdir -I ${ni} -p $sp -Z 4 -P 16 -g \
    #     -s 1000 -M foreground
    ../scripts/train.py -i $image -o $outdir -I 6000 -Z 4 \
        -s 200 -p ${sp} -g -e 100 -l 5e-4 -d 1e-3 -b 128 -u 10 -D 2
done
