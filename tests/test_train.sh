...
images=(/data/phantom/simu/SUPERRES-ADNIPHANTOM_20200711_PHANTOM-T2-TSE-3D-CORONAL-PRE-ACQ1-01mm_resampled_rot-0-45-45_type-gauss_fwhm-2p0_scale-0p125_len-13.nii)

for image in ${images[@]}; do
    fwhm=$(echo $image | sed "s/.*\(fwhm-.*\)_scale.*/\1/")
    scale=$(echo $image | sed "s/.*\(scale-.*\)_len.*/\1/")
    len=$(echo $image | sed "s/.*\(len-.*\)\.nii/\1/")
    outdir=../tests/results_31-31-31_1024/simu_${fwhm}_${scale}_${len}
    ...
    docker run --gpus device=1 --rm \
        ...
        -sw 0 -wd 1e-2 -lrdk 3,1 3,1 3,1 1,1 1,1 -lrdc 1024 1024 1024 1024
done # | rush -j 3 {}
