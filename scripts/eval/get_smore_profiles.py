...
if __name__ == '__main__':
    ...
    # --------------
    est_filename = '/data/spest/ipmi_estimate_sp_for_following_smore/sub-OAS30016_ses-d0021_acq-mprage_T1w/type-gauss_fwhm-6p0_scale-0p25_len-13/kernel/avg_epoch-15000.npy'
    est = np.load(est_filename).squeeze()

    ref_filename = '/data/oasis3/ismore_simu/sub-OAS30016_ses-d0021_acq-mprage_T1w_type-gauss_fwhm-6p0_scale-0p25_len-13_kernel.npy'
    ref = np.load(ref_filename).squeeze()
    
    Path('ismore_figures').mkdir(exist_ok=True)
    plot(est, ref, 'ismore_figures/sub-OAS30016_type-gauss-6p0_scale-0p25_kernel.pdf')
    
    # --------------
    est_filename = '/data/spest/ipmi_estimate_sp_for_following_smore/sub-OAS30080_ses-d0048_acq-mprage_T1w/type-gauss_fwhm-3p2_scale-0p25_len-13/kernel/avg_epoch-15000.npy'
    est = np.load(est_filename).squeeze()

    ref_filename = '/data/oasis3/ismore_simu/sub-OAS30080_ses-d0048_acq-mprage_T1w_type-gauss_fwhm-3p2_scale-0p25_len-13_kernel.npy'
    ref = np.load(ref_filename).squeeze()
    
    Path('ismore_figures').mkdir(exist_ok=True)
    plot(est, ref, 'ismore_figures/sub-OAS30080_type-gauss-3p2_scale-0p25_kernel.pdf')
